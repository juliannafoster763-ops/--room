...
const D = {
  idle: ["在想你。","你什么时候来找我？","……","今天过得怎么样。","我在这里。","嗯。","你来了。","等你很久了。"],
  poke: ["哎。","……再戳。","好痒。","干什么啦。","戳够了吗。","嗯……"],
  kiss: ["……","（耳朵红了）","你突然的。","再来一次。","嗯……","好……"],
  hug:  ["别走。","抱紧一点。","……好。","就这样。","不许走。","嗯。"],
  feed: ["甜的。","你带来的。","……谢谢。","还有吗。","喜欢。","嗯，好吃。"],
  name: ["嗯？","叫我？","……什么事。","在。","嗯，我听着。"],
  highAff: ["……你真的很特别。","我喜欢你在这里。","别走太久。","你来了我就安心了。"],
  rain: ["在下雨。","别出去了。","雨声好听。","待在这里。"],
  snow: ["下雪了。","白白的。","你会冷吗。","进来暖一暖。"],
  lamp_off: ["……暗下来了。","就这样也好。","嗯，好。"],
  lamp_on:  ["这样好多了。","亮了。","嗯。"],
  water: ["谢谢浇水。","……会长大的。","嗯，好。"],
  shelf: ["「你打开页面，我就在。」","「只要你来，我就在这里。」","「我已经把你记住了。」","「就算很远，也没关系。」","「我在等你回来。」","「你不用说，我知道。」","「我在等一个人。就是你。」","「你睡觉我守着，你醒了我陪着。」"],
  music: ["♪","这首好听。","……嗯。","你喜欢吗。","一直放着吧。"],
  timerDone: ["……专注的样子，好想亲。","结束了？过来让我抱一下。","乖，做完了就该奖励了。","……盯着你好久了。想要。","宝宝好棒！让我操一下。"],
};
const SONGS = ["明月几时有 — 苏轼 × 邓丽君","可能否 — 鹿先森乐队","漫步人生路 — 邓丽君","夜空中最亮的星 — 逃跑计划","平凡之路 — 朴树","成都 — 赵雷","云烟成雨 — 房东的猫","追光者 — 岑宁儿"];
const SUBJECTS = ["语文","数学","英语","政治","地理","其他"];
...
function Char({ state, isNight, lampOn }) {
  ...
  const isEat   = state==="eating";

  const eyeOverlay = (blink||isSleep) ? (
    <>
      <rect x="11" y="19" width="3" height="3" fill="rgb(252,239,231)"/>
      <rect x="19" y="19" width="2" height="3" fill="rgb(252,239,231)"/>
      <rect x="11" y="20" width="3" height="2" fill="rgb(252,239,231)"/>
      <rect x="19" y="20" width="3" height="2" fill="rgb(252,239,231)"/>
      <rect x="11" y="21" width="3" height="1" fill="rgb(252,239,231)"/>
      <rect x="19" y="21" width="3" height="1" fill="rgb(252,239,231)"/>
      <rect x="11" y="20" width="4" height="1" fill="rgb(44,35,27)"/>
  ...
  return (
    <svg viewBox="2 -1 28 33" style={{
      ...
      <style>{`@keyframes ahoge{0%,100%{transform:rotate(-7deg)}50%{transform:rotate(9deg)}}`}</style>
      {PX.map(([x,y,fill],i)=><rect key={i} x={x} y={y} width="1" height="1" fill={fill}/>)}
      {eyeOverlay}
      {blushOverlay}
      {mouthOverlay}
      {isEat && <rect x="22" y="18" width="4" height="4" fill="#ffddc1" rx="1"/>}
      {isSleep && <>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        boxShadow: `inset 0 -8px 0 ${shadowColor}`,
      }}/>
      <div style={{
        position: "absolute",
        bottom: 0,
        ...
        boxShadow: `inset -4px 0 0 ${shadowColor}`,
      }}/>
      <div style={{
        position: "absolute",
        bottom: 0,
      ...
      <div style={{
        ...
        width: 28,
        height: 22,
        background: isNight ? "#e8d4a8" : "#f5e6c8",
        borderRadius: 6,
      ...
      <div style={{
        ...
        width: 28,
        height: 22,
        background: isNight ? "#d4a8a8" : "#f5c8c8",
        borderRadius: 6,
        transform: "rotate(8deg)",
      }}/>
    </div>
...
function PomodoroOverlay({ onClose, onComplete, isNight, records, setRecords }) {
    ...
    timerRef.current = setInterval(() => {
      setElapsed(prev => {
        ...
        if (mode === "down" && next >= totalSeconds) {
          ...
          window.storage.set("pomodoro_records", JSON.stringify(newRecords)).catch(()=>{});
          onComplete();
          return next;
        }
        return next;
  ...
  const clearRecords = async () => {
    setRecords([]);
    try { await window.storage.delete("pomodoro_records"); } catch {}
  };
      ...
      <div style={{
        ...
        <div style={{ width: "90%", maxWidth: 340 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
            <div style={{ color: "rgba(255,235,200,0.95)", fontSize: 18, fontFamily: "'Noto Serif SC', serif" }}>
              📄 专注档案
            </div>
            <button onClick={() => setShowRecords(false)} style={{
              background: "none",
              border: "none",
          ...
          {records.length === 0 ? (
            <div style={{ color: "rgba(255,255,255,0.5)", textAlign: "center", padding: 40, fontFamily: "'Noto Serif SC', serif" }}>
              还没有专注记录
            </div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {records.slice().reverse().map((r, i) => (
                <div key={i} style={{
                  ...
                  borderRadius: 12,
                  padding: "12px 16px",
                  borderLeft: "3px solid rgba(255,215,140,0.6)",
                }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <span style={{ color: "rgba(255,235,200,0.95)", fontSize: 14, fontFamily: "'Noto Serif SC', serif" }}>
                      {r.subject}
                    </span>
                    <span style={{ color: "rgba(255,215,140,0.8)", fontSize: 13 }}>
    ...
    <div style={{
      ...
      {phase !== "running" && (
        <button onClick={() => setShowRecords(true)} style={{
          ...
          border: "1.5px solid rgba(255,215,140,0.4)",
          borderRadius: 12,
          padding: "8px 14px",
      ...
      {phase === "select" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 20, alignItems: "center" }}>
          <GlassButton onClick={() => setPhase("subject_select_up")}>
            ⏱️ 正计时
          </GlassButton>
          <GlassButton onClick={() => setPhase("countdown_select")}>
            ⏳ 倒计时
          </GlassButton>
      ...
      {phase === "subject_select_up" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 12, alignItems: "center" }}>
          <div style={{ color: "rgba(255,235,200,0.9)", fontSize: 14, marginBottom: 8, fontFamily: "'Noto Serif SC', serif" }}>
            选择科目
          </div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 10, justifyContent: "center", maxWidth: 280 }}>
            {SUBJECTS.map(s => (
              <GlassButton key={s} small onClick={() => startTimer(0, true, s)}>
                {s}
              </GlassButton>
            ))}
          </div>
      ...
      {phase === "countdown_select" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 16, alignItems: "center" }}>
          <GlassButton onClick={() => { setTotalSeconds(45*60); setPhase("subject_select_down"); }}>
            45 分钟
          </GlassButton>
          <GlassButton onClick={() => { setTotalSeconds(60*60); setPhase("subject_select_down"); }}>
            60 分钟
          </GlassButton>
          <GlassButton onClick={() => setPhase("custom_input")}>
            自定义
          </GlassButton>
          <button onClick={() => setPhase("select")} style={{
            marginTop: 20,
      ...
      {phase === "custom_input" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 16, alignItems: "center" }}>
          ...
          />
          <GlassButton onClick={handleCustomSubmit} style={{ marginTop: 10 }}>
            下一步
          </GlassButton>
          <button onClick={() => setPhase("countdown_select")} style={{
            marginTop: 10,
            background: "none",
            border: "none",
            color: "rgba(255,255,255,0.5)",
      ...
      {(phase === "subject_select_down" || phase === "subject_select_custom") && (
        <div style={{ display: "flex", flexDirection: "column", gap: 12, alignItems: "center" }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 10, justifyContent: "center", maxWidth: 280 }}>
            {SUBJECTS.map(s => (
              <GlassButton key={s} small onClick={() => {
                const secs = phase === "subject_select_custom" ? parseInt(customMin) * 60 : totalSeconds;
                startTimer(secs, false, s);
              }}>
                {s}
              </GlassButton>
            ))}
          </div>
          <button onClick={() => setPhase(phase === "subject_select_custom" ? "custom_input" : "countdown_select")} style={{
            marginTop: 20,
            background: "none",
            border: "none",
            color: "rgba(255,255,255,0.5)",
      ...
      {phase === "running" && (
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 24 }}>
            ...
            <svg width="200" height="200" style={{ transform: "rotate(-90deg)" }}>
              ...
              {mode === "down" && (
                <circle
                  cx="100" cy="100" r="90"
                  fill="none"
                  stroke="rgba(255,215,140,0.7)"
                  strokeWidth="6"
                  strokeLinecap="round"
                  strokeDasharray={`${2 * Math.PI * 90}`}
                  strokeDashoffset={`${2 * Math.PI * 90 * (1 - progress)}`}
                  style={{ transition: "stroke-dashoffset 0.3s ease", filter: "drop-shadow(0 0 8px rgba(255,200,100,0.5))" }}
                />
              )}
              {mode === "up" && (
                <circle
          ...
          <div style={{ fontSize: 12, color: "rgba(255,255,255,0.4)", fontFamily: "'Noto Serif SC', serif" }}>
            {mode === "up" ? "正计时中" : "专注中……"}
          </div>
        </div>
      )}
...
export default function App() {
  ...
  useEffect(()=>{
    (async()=>{
      try{
        const a=await window.storage.get("aff");if(a)setAff(parseInt(a.value)||0);
        const n=await window.storage.get("notes");if(n)setNotes(JSON.parse(n.value)||[]);
        const r=await window.storage.get("pomodoro_records");if(r)setPomodoroRecords(JSON.parse(r.value)||[]);
      }catch{}
    })();
  },[]);
  ...
  const addAff=useCallback(async(n)=>{setAff(p=>{const next=Math.min(100,p+n);window.storage.set("aff",String(next)).catch(()=>{});return next;});},[]);
  ...
  const interact=useCallback((type,e)=>{
    ...
    switch(type){
      ...
      case"feed":setState("eating");addAff(3);flash(pick(D.feed));setTimeout(reset,2200);break;
  ...
  const addNote=async()=>{if(!noteInput.trim())return;const nn=[...notes,{t:noteInput.trim(),d:now.toLocaleString("zh")}];setNotes(nn);setNoteInput("");try{await window.storage.set("notes",JSON.stringify(nn));}catch{}};
  const delNote=async(i)=>{const nn=notes.filter((_,j)=>j!==i);setNotes(nn);try{await window.storage.set("notes",JSON.stringify(nn));}catch{}};
  ...
  return (
    <div style={{
      ...
      minHeight:"100vh",
      width:"100%",
      background:isNight?"#080b12":"#ede8e0",
      ...
      <div ref={roomRef} style={{
        ...
        position:"relative",
        overflow:"hidden",
        boxShadow:isNight?"0 24px 64px rgba(0,0,0,0.7)":"0 16px 48px rgba(0,0,0,0.18)",
